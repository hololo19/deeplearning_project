...
# Select single asset for training (e.g., BTC)
asset_name = 'BTC'  # Can be changed to 'DPZ', 'BTC', 'NFLX'

# Use sequential data for Bidirectional LSTM training, but select only one asset
...
# Prepare data for single asset (BTC is column 3, index 2)
asset_index = {'AMZN': 0, 'DPZ': 1, 'BTC': 2, 'NFLX': 3}[asset_name]

y_train_seq_single = y_train_seq_full[:, asset_index:asset_index+1]  # Keep 2D array
y_test_seq_single = y_test_seq_full[:, asset_index:asset_index+1]
scaler_y_single = MinMaxScaler()

# Scale data for single asset
...
# Create sequences for single asset historical prices
...
def create_bidirectional_lstm_model(input_shape):
    """
    Create Bidirectional LSTM model for single asset
    """
    model = Sequential()
...
model = create_bidirectional_lstm_model(input_shape)
...
# Save model
model.save(f'/home/holo/market_pre/bidirectional_lstm_{asset_name.lower()}_model.keras')
print(f"\nModel saved to: /home/holo/market_pre/bidirectional_lstm_{asset_name.lower()}_model.keras")

print(f"\n{asset_name} Bidirectional LSTM model training and prediction completed!")